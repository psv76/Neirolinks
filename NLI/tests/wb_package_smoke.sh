...
if [ "$1" = first ]; then
    ...
    test "$(nli --version)" = 0.1.4
    apt-get install -y /packages/neiro-nli_0.1.5_all.deb
    sha256sum -c /evidence/old-conffile.sha256
    test "$(nli --version)" = 0.1.5
    ...
    apt-get install --reinstall -y /packages/neiro-nli_0.1.5_all.deb
