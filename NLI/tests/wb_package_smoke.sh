...
if [ "$1" = first ]; then
    ...
    apt-get install -y /old/neiro-nli_0.1.8_all.deb
    test "$(nli_version)" = 0.1.8
    apt-get install -y /packages/neiro-nli_0.1.9_all.deb
    sha256sum -c /evidence/old-conffile.sha256
    test "$(nli_version)" = 0.1.9
    ...
    apt-get install --reinstall -y /packages/neiro-nli_0.1.9_all.deb
