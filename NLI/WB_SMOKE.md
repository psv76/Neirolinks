# NLI 0.1.4: bootstrap и восстановление после FIT
...
## Проверка пакета и upgrade 0.1.0 → 0.1.4

Использовать `neiro-nli_0.1.4_all.deb` из artifact `neiro-nli-0.1.4-deb`
...
Ожидание: версия `0.1.4`, status `ok`, timestamps сервисов не изменились.
...
NLI 0.1.4 не содержит conffiles: config создаётся инженером, хранится отдельно
...
sha256sum -c neiro-nli_0.1.4_all.deb.sha256
apt install --reinstall ./neiro-nli_0.1.4_all.deb
nli --version
nli --json status
```
...
0.1.4 выдаёт `LEGACY_MIGRATION_REQUIRED` до bootstrap. Инженер отдельно:
