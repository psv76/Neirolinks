...
# NLI 0.1.8: bootstrap и восстановление после FIT
...
## Проверка пакета и upgrade 0.1.0 → 0.1.8

Использовать `neiro-nli_0.1.8_all.deb` из artifact `neiro-nli-0.1.8-deb`
...
Ожидание: версия `0.1.8`, status `ok`, timestamps сервисов не изменились.
...
NLI 0.1.8 не содержит conffiles: config создаётся инженером, хранится отдельно
...
sha256sum -c neiro-nli_0.1.8_all.deb.sha256
apt install --reinstall ./neiro-nli_0.1.8_all.deb
nli --version
nli --json status
```
...
0.1.8 выдаёт `LEGACY_MIGRATION_REQUIRED` до bootstrap. Инженер отдельно:
