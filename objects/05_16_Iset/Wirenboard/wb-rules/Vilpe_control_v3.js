// Управление вытяжкой Vilpe v.3.3
// Объект: 05 16 Исеть
//
// Назначение:
// - управляет вытяжкой Vilpe относительно фактической скорости Breezart;
// - в норме Vilpe работает ниже Breezart на заданное отставание;
// - при повышенной влажности отставание уменьшается;
// - при высокой влажности применяется фиксированная уставка;
// - режим сушки сауны оставлен в логике, но по умолчанию отключён,
//   потому что MAP12E с токами сауны неисправен и отключён.
//
// Пишет:
// - wb-mao4_220/Channel 2 Switch
// - wb-mao4_220/Channel 2 Dimming Level
// - при включённой координации сушки также пишет в дополнительные каналы WB-MAO4.
//
// Важно:
// - один физический канал должен иметь одного писателя;
// - не включать автосушку сауны по токам, пока MAP12E не восстановлен;
// - запись в Vilpe ограничена: уставка округляется до 1%, повторная запись
//   выполняется только при значимом изменении, смене режима, смене питания
//   или контрольном интервале 5 минут.
...
var VC2_CFG = {
  ids: {
    ...
    userTitle: "Управление вытяжкой Vilpe v.3.3",
  ...
  defaults: {
    ...
    // MAP12E с токами сауны на объекте отключён.
    // Если счётчик восстановят, включить saunaCurrentsEnabled через виртуальное устройство.
    saunaCurrentsEnabled: false,

    // Ограничение частых записей в аналоговый выход Vilpe.
    vilpeWriteThresholdPct: 1,
    vilpeForceRewriteSec: 300,
...
var vc2State = {
  ...
  lastLoggedActiveMode: "",
  lastLoggedVilpeSetpoint: null,
  lastLoggedSafeReason: "",
  lastLoggedHumiditySensorsError: false,
  lastLoggedBreezartDataError: false,
  lastLoggedSaunaCurrentsError: false,
  lastLoggedDryingCurrentsMissing: false,
  lastLoggedSaunaCurrentsEnabled: null,

  lastAppliedActiveMode: "",
  lastVilpeWriteTs: 0
};
...
// ============================================================
// 3.1. ЛОГИРОВАНИЕ В СИСТЕМНЫЙ ЖУРНАЛ WB-RULES
// ============================================================

function vc2FormatLogParams(params) {
  var list = [];
  var k;
...
function vc2WriteLog(eventName, eventText, params) {
  var message = "[вентиляция][vilpe_control_v3][vilpe]; " +
    eventName + "=" + eventText + vc2FormatLogParams(params);
...
function vc2LogModeChange(activeMode, hSnap, bSnap) {
  if (vc2State.lastLoggedActiveMode === activeMode) {
    return;
  }

  vc2WriteLog("СОСТОЯНИЕ", "Режим изменён", {
    "режим": vc2ModeName(activeMode),
    "макс_влажность": vc2LogValue(hSnap.maxHumidity),
    "датчик": hSnap.maxHumiditySensor,
    "breezart_факт": bSnap.actualOk ? vc2LogValue(bSnap.actual) : "нет",
    "breezart_уставка": bSnap.setpointOk ? vc2LogValue(bSnap.setpoint) : "нет"
  });

  vc2State.lastLoggedActiveMode = activeMode;
}

function vc2LogSafeCleared() {
  if (vc2State.lastLoggedSafeReason === "") {
    return;
  }

  vc2WriteLog("СОСТОЯНИЕ", "SAFE-режим снят", {
    "причина_была": vc2State.lastLoggedSafeReason
  });

  vc2State.lastLoggedSafeReason = "";
}

function vc2LogFlagChanges(flags, alarmText) {
  if (flags.humiditySensorsError !== vc2State.lastLoggedHumiditySensorsError) {
    vc2WriteLog(flags.humiditySensorsError ? "ОШИБКА" : "СОСТОЯНИЕ", flags.humiditySensorsError ? "Ошибка датчиков влажности" : "Датчики влажности восстановлены", {
      "причина": flags.humiditySensorsError ? "один или несколько датчиков недоступны" : "ошибка снята"
    });
    vc2State.lastLoggedHumiditySensorsError = flags.humiditySensorsError;
  }

  if (flags.breezartDataError !== vc2State.lastLoggedBreezartDataError) {
    vc2WriteLog(flags.breezartDataError ? "ОШИБКА" : "СОСТОЯНИЕ", flags.breezartDataError ? "Ошибка данных Breezart" : "Данные Breezart восстановлены", {
      "причина": flags.breezartDataError ? vc2SafeText(alarmText) : "ошибка снята"
    });
    vc2State.lastLoggedBreezartDataError = flags.breezartDataError;
  }

  if (flags.saunaCurrentsError !== vc2State.lastLoggedSaunaCurrentsError) {
    vc2WriteLog(flags.saunaCurrentsError ? "ОШИБКА" : "СОСТОЯНИЕ", flags.saunaCurrentsError ? "Ошибка токов сауны" : "Токи сауны восстановлены", {
      "причина": flags.saunaCurrentsError ? "токи L1/L2/L3 недоступны или невалидны" : "ошибка снята"
    });
    vc2State.lastLoggedSaunaCurrentsError = flags.saunaCurrentsError;
  }

  if (flags.dryingCurrentsMissing !== vc2State.lastLoggedDryingCurrentsMissing) {
    vc2WriteLog(flags.dryingCurrentsMissing ? "ОШИБКА" : "СОСТОЯНИЕ", flags.dryingCurrentsMissing ? "Во время сушки нет токов" : "Токи во время сушки восстановлены", {
      "причина": flags.dryingCurrentsMissing ? "сушка продолжается в grace-период" : "ошибка снята"
    });
    vc2State.lastLoggedDryingCurrentsMissing = flags.dryingCurrentsMissing;
  }
}

function vc2LogSaunaCurrentsSetting(settings) {
  if (vc2State.lastLoggedSaunaCurrentsEnabled === settings.saunaCurrentsEnabled) {
    return;
  }

  vc2WriteLog("СОСТОЯНИЕ", settings.saunaCurrentsEnabled ? "Контроль токов сауны включён" : "Контроль токов сауны отключён", {
    "причина": settings.saunaCurrentsEnabled ? "MAP12E используется для автосушки" : "MAP12E неисправен или отключён, автосушка недоступна"
  });

  if (!settings.saunaCurrentsEnabled) {
    vc2State.lastLoggedSaunaCurrentsError = false;
    vc2State.lastLoggedDryingCurrentsMissing = false;
  }

  vc2State.lastLoggedSaunaCurrentsEnabled = settings.saunaCurrentsEnabled;
}
...
function vc2GetSettings() {
  ...
  s.saunaCurrentsEnabled = !!vc2ReadRaw(vc2UserCell("sauna_currents_enabled"));

  s.vilpeWriteThresholdPct = VC2_CFG.defaults.vilpeWriteThresholdPct;
  s.vilpeForceRewriteSec = VC2_CFG.defaults.vilpeForceRewriteSec;
  ...
  s.vilpeWriteThresholdPct = vc2Clamp(s.vilpeWriteThresholdPct, 0, 20);
  s.vilpeForceRewriteSec = vc2Clamp(s.vilpeForceRewriteSec, 5, 3600);
...
function vc2GetCurrentsSnapshot(settings) {
  var enabled = true;

  if (settings && settings.saunaCurrentsEnabled === false) {
    enabled = false;
  }
...
function vc2SelectBreezartBase(bSnap) {
  if (bSnap.actualOk) {
    return { ok: true, value: bSnap.actual, source: "факт Breezart" };
  }

  if (bSnap.setpointOk) {
    return { ok: true, value: bSnap.setpoint, source: "уставка Breezart" };
  }

  return { ok: false, value: null, source: "" };
}
...
function vc2ComputeVilpeTarget(activeMode, settings, bSnap) {
  ...
  if (activeMode === "DRYING") {
    ...
    baseSource = "фиксированная уставка сушки";
    baseValue = settings.dryingSetpoint;
  } else if (activeMode === "VERY_HIGH_HUMIDITY") {
    target = settings.veryHighSetpoint;
    baseSource = "фиксированная уставка высокой влажности";
    baseValue = settings.veryHighSetpoint;
  } else if (activeMode === "HIGH_HUMIDITY") {
    base = vc2SelectBreezartBase(bSnap);
    if (!base.ok) {
      reason = "Нет факта и уставки Breezart для режима повышенной влажности";
      return { ok: false, target: null, reason: reason, baseSource: "", baseValue: null, lag: 0 };
    }
    lag = settings.highHumidityLag;
    baseSource = base.source;
    baseValue = base.value;
    target = base.value - lag;
  } else {
    base = vc2SelectBreezartBase(bSnap);
    if (!base.ok) {
      reason = "Нет факта и уставки Breezart для нормального режима";
      return { ok: false, target: null, reason: reason, baseSource: "", baseValue: null, lag: 0 };
    }
    lag = settings.normalLag;
    baseSource = base.source;
    baseValue = base.value;
    target = base.value - lag;
  }
...
function vc2StartDrying(reasonText, settings) {
  ...
  vc2WriteLog("СОСТОЯНИЕ", "Сушка сауны запущена", {
    "причина": vc2SafeText(reasonText),
    "длительность_мин": settings.dryingDurationMin,
    "уставка_vilpe": settings.dryingSetpoint
  });

  vc2WriteLog("КОМАНДА", "Установить уровни сушки", {
    "причина": vc2SafeText(reasonText),
    "канал": vc2GetManagedChannels(settings).join(", "),
    "значение в канал": "Vilpe=" + settings.dryingSetpoint + "%, доп. каналы по сценарию"
  });
...
function vc2StopDrying(reasonText) {
  ...
  vc2WriteLog("СОСТОЯНИЕ", "Сушка сауны остановлена", {
    "причина": vc2SafeText(reasonText)
  });

  vc2WriteLog("КОМАНДА", "Восстановить уровни после сушки", {
    "причина": vc2SafeText(reasonText),
    "канал": vc2SerializeSavedLevels(vc2State.savedLevels)
  });
...
function vc2UpdateDryingState(cSnap, settings) {
  ...
  if (!settings.saunaCurrentsEnabled) {
    if (vc2State.dryingActive) {
      vc2StopDrying("Контроль токов сауны отключён");
      return;
    }
  ...
  if (cSnap.ok) {
    if (vc2State.currentsMissingSinceTs !== 0) {
      vc2WriteLog("СОСТОЯНИЕ", "Токи сауны восстановлены", {
        "пауза_сек": Math.floor((now - vc2State.currentsMissingSinceTs) / 1000)
      });
    }
    vc2State.currentsMissingSinceTs = 0;

    if (vc2CurrentIsSaunaOn(cSnap, settings)) {
      if (!vc2State.saunaWasOn) {
        vc2WriteLog("СОСТОЯНИЕ", "Сауна включена по токам", {
          "L1": vc2LogValue(cSnap.l1),
          "L2": vc2LogValue(cSnap.l2),
          "L3": vc2LogValue(cSnap.l3),
          "порог": settings.saunaOnCurrentA
        });
      }
  ...
  if (!vc2State.dryingActive) {
    // Если сушка не активна, просто ждём восстановления данных.
    vc2State.dryingConditionSinceTs = 0;
    vc2PersistState();
    return;
  }
  ...
  if (vc2State.currentsMissingSinceTs === 0) {
    ...
    vc2WriteLog("ОШИБКА", "Потеря токов во время сушки", {
      "причина": "ожидание восстановления в пределах допуска",
      "допуск_сек": settings.currentsLossGraceSec
    });
    vc2PersistState();
    return;
  }
...
function vc2ApplyVilpeSetpoint(target, activeMode, reasonText, targetInfo) {
  var t = vc2RoundPercent(vc2Clamp(target, 0, 100));
  var prev = vc2State.lastAppliedVilpeSetpoint;
  var now = vc2Now();
  var desiredPower = t > 0;
  var currentPower = !!vc2ReadRaw(VC2_CFG.vilpe.enableSwitch);
  var elapsedSec = vc2State.lastVilpeWriteTs > 0 ? Math.floor((now - vc2State.lastVilpeWriteTs) / 1000) : 999999;
  var writeReason = "";

  vc2Write(vc2UserCell("target_vilpe_setpoint"), t);

  if (prev === null) {
    writeReason = "первая запись";
  } else if (Math.abs(prev - t) >= VC2_CFG.defaults.vilpeWriteThresholdPct) {
    writeReason = "уставка изменилась минимум на " + VC2_CFG.defaults.vilpeWriteThresholdPct + "%";
  } else if (vc2State.lastAppliedActiveMode !== activeMode) {
    writeReason = "изменился режим";
  } else if (currentPower !== desiredPower) {
    writeReason = "состояние питания не совпадает с расчётом";
  } else if (elapsedSec >= VC2_CFG.defaults.vilpeForceRewriteSec) {
    writeReason = "контрольная повторная запись 5 минут";
  }
  ...
  vc2Write(VC2_CFG.vilpe.enableSwitch, desiredPower);
  vc2State.lastAppliedVilpeSetpoint = t;
  vc2State.lastAppliedActiveMode = activeMode;
  vc2State.lastVilpeWriteTs = now;

  vc2WriteLog("КОМАНДА", "Установить скорость Vilpe", {
    "причина": vc2SafeText(reasonText) + ", " + writeReason,
    "режим": vc2ModeName(activeMode),
    "канал": VC2_CFG.vilpe.speed + ", " + VC2_CFG.vilpe.enableSwitch,
    "значение в канал": t + "%, питание=" + (desiredPower ? "ON" : "OFF"),
    "предыдущее_значение": prev === null ? "нет" : prev,
    "база": targetInfo && targetInfo.baseSource ? targetInfo.baseSource : "",
    "база_значение": targetInfo && targetInfo.baseValue !== null ? vc2LogValue(targetInfo.baseValue) : "",
    "отставание": targetInfo ? targetInfo.lag : ""
  });
...
function vc2ApplySafeBehavior(reasonText, bSnap) {
  ...
  var safeReason = vc2SafeText(reasonText);

  if (vc2State.lastLoggedSafeReason !== safeReason) {
    vc2WriteLog("АВАРИЯ", "SAFE-режим", {
      "причина": safeReason,
      "уставка_vilpe": safeSetpoint
    });
    vc2State.lastLoggedSafeReason = safeReason;
  }

  vc2ApplyVilpeSetpoint(safeSetpoint, "NORMAL", "SAFE-режим: " + safeReason, {
    baseSource: "безопасная уставка",
    baseValue: safeSetpoint,
    lag: 0
  });
  vc2State.lastErrorText = safeReason;
  ...
  vc2Write(vc2ServiceCell("event_text"), "SAFE-режим: " + safeReason);
}
...
function vc2UpdateInputAvailabilityText(hSnap, bSnap, cSnap) {
  ...
  if (cSnap.enabled === false) {
    parts.push("токи_сауны=отключены");
  } else {
...
function vc2EvaluateAndApply() {
  ...
  var cSnap = vc2GetCurrentsSnapshot(settings);
  ...
  vc2LogSaunaCurrentsSetting(settings);
  vc2UpdateDryingState(cSnap, settings);

  if (!bSnap.actualOk && !bSnap.setpointOk) {
    flags.breezartDataError = true;
  } else if (bSnap.actualOk) {
    vc2State.lastValidBreezartSetpoint = bSnap.actual;
  } else {
    vc2State.lastValidBreezartSetpoint = bSnap.setpoint;
  }

  if (settings.saunaCurrentsEnabled && !cSnap.ok) {
  ...
  vc2LogModeChange(activeMode, hSnap, bSnap);
  ...
  if (!humidityReliable && !vc2State.dryingActive) {
    ...
    if (target.ok) {
      vc2LogSafeCleared();
      vc2ApplyVilpeSetpoint(target.target, activeMode, "расчёт по активному режиму", target);
  ...
  if (!settings.saunaCurrentsEnabled) {
    vc2Write(vc2UserCell("drying_status"), "Недоступна: MAP12E отключён");
  } else {
  ...
  vc2LogFlagChanges(flags, alarmText);
  vc2UpdateTimerLeft();
}
...
defineVirtualDevice(VC2_CFG.ids.userVdev, {
  ...
  cells: {
    ...
    sauna_currents_enabled: {
      type: "switch",
      value: VC2_CFG.defaults.saunaCurrentsEnabled,
      title: "Автосушка сауны по токам MAP12E",
      order: 15
    },
...
var vc2TrackedInputs = [
  ...
  vc2UserCell("sauna_currents_enabled")
];

defineRule("vilpe_control_v3_on_change", {
...
function vc2RestoreStateOnStartup() {
  ...
  if (vc2State.dryingActive) {
    if (!settings.saunaCurrentsEnabled) {
      ...
      vc2State.dryingConditionSinceTs = 0;
      vc2State.saunaWasOn = false;
      vc2State.dryingStopReason = "Сушка сброшена при старте: MAP12E отключён";
      vc2Write(vc2UserCell("drying_stop_reason"), vc2State.dryingStopReason);
      vc2Write(vc2ServiceCell("event_text"), vc2State.dryingStopReason);
    } else if (vc2State.dryingUntilTs <= now) {
      vc2State.dryingActive = false;
...
function vc2StartupDiagnostics() {
  ...
  if (!bSnap.actualOk && !bSnap.setpointOk) {
    errors.push("Недоступны факт и уставка Breezart");
  }
  ...
  if (errors.length > 0) {
    ...
    vc2WriteLog("ОШИБКА", "Стартовая диагностика: есть замечания", {
      "причина": errors.join("; ")
    });
  } else {
    ...
    if (!settings.saunaCurrentsEnabled) {
      vc2Write(vc2ServiceCell("event_text"), "Стартовая диагностика OK; автосушка сауны отключена, MAP12E не используется");
    }
    vc2WriteLog("СКРИПТ", "Стартовая диагностика OK", {
      "автосушка_сауны": settings.saunaCurrentsEnabled ? "включена" : "отключена"
    });
  }
...
setTimeout(function () {
  vc2WriteLog("СКРИПТ", "Запущен", {
    "версия": "3.3",
    "канал_vilpe": VC2_CFG.vilpe.speed,
    "питание_vilpe": VC2_CFG.vilpe.enableSwitch,
    "breezart_факт": VC2_CFG.breezart.actual,
    "breezart_уставка": VC2_CFG.breezart.setpoint,
    "MAP12E": "автосушка по токам отключена по умолчанию"
  });
