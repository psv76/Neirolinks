...
/*
 Список номеров для SMS.
 Формат проверить по фактической работе модема.
 Обычно:
 +79XXXXXXXXX
*/
SYS.smsPhones = [
...
function startsWithEmojiPrefix(text)
{
    var s = trimText(text);

    return s.indexOf("⚠️") === 0 ||
           s.indexOf("✅") === 0 ||
           s.indexOf("ℹ️") === 0;
}
...
function buildMessage(params)
{
    ...
    objectName = isNonEmptyString(params.objectName) ? trimText(params.objectName) : SYS.objectName;
...
function sendSMSToPhone(phone, message)
{
    ...
    if (!isNonEmptyString(phone) || phone === "+79000000000")
    {
...
function sendSMS(message)
{
    ...
    for (i = 0; i < phones.length; i++)
    {
        ...
        if (sendSMSToPhone(phone, message))
            sent++;
    }
...
function sendAlert(buildingName, eventText, detailsText, recommendationText)
{
    return sendMessage({
        ...
        eventText: "⚠️ " + trimText(eventText || ""),
...
function sendInfo(buildingName, eventText, detailsText, recommendationText)
{
    return sendMessage({
        buildingName: buildingName,
        eventText: startsWithEmojiPrefix(eventText) ? trimText(eventText) : "ℹ️ " + trimText(eventText || ""),
        detailsText: detailsText,
        recommendationText: recommendationText
    });
}
