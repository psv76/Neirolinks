...
var SCRIPT = '502_gp_dom_manager';
var VD = 'hm2_502_gp_dom';

var CH = {
    pump: 'A03/K2',
    valveEnable: 'A05/Channel 2 Switch',
    valvePosition: 'A05/Channel 2 Dimming Level',
    supply: 'wb-m1w2_167/External Sensor 1', // 415
    ret: 'wb-m1w2_167/External Sensor 2',    // 416
    source: 'wb-m1w2_170/External Sensor 1', // 411
    zones: ['A13/K1', 'A13/K2', 'A13/K3', 'A13/K4', 'A13/K5', 'A13/K6',
        'A14/K1', 'A14/K2', 'A14/K3']
};

// Собственные предварительные параметры 502, НЕ подтверждённые настройки плитки.
// Карта содержит температуры пола/воздуха, а не подачи теплоносителя 415.
// Target, hardMax, весь tuning и response watchdog требуют отдельного ПНР.
var CFG = {
    periodS: 30,
    requestTtlS: 90,
    actuatorDelayS: 180,
    targetSupplyC: 32, // Осторожный старт для shadow/будущего ПНР, не объектная истина.
    targetMinC: 20,
    targetMaxC: 35,
    ...
    tuning: {
        ...
        minPosPct: 0,
        maxPosPct: 100,
        hardMaxC: 40, // Предварительный предел теплоносителя 415; требует ПНР.
        supplyValidMinC: -20,
        supplyValidMaxC: 80,
        sensorBadLimit: 3,
...
{
    var msg = '[отопление][' + SCRIPT + '][502 ГП дом]; ' + eventName + '=' + text;
    if (level === 'error')
        log.error(msg);
...
function getTarget()
{
    var v = clamp(dev[VD + '/target_supply_c'], CFG.targetMinC, CFG.targetMaxC);
...
function evaluate(reason)
{
    ...
    for (i = 0; i < CH.zones.length; i++) groups.push(rb(dev[CH.zones[i]]));
...
defineVirtualDevice(VD, {
    ...
    cells: {
        ...
        target_supply_c: { title: 'ПНР 502: цель подачи ГП, °C', type: 'range', value: CFG.targetSupplyC, min: CFG.targetMinC, max: CFG.targetMaxC, order: 50 },
        ...
        reset_fault: { title: 'Сброс аварии 502', type: 'pushbutton', order: 100 },
        ...
        active_zone_count: { title: 'Открытых зон ГП', type: 'value', value: 0, readonly: true, order: 400 },
        zone_demand: { title: 'Есть запрос зон', type: 'switch', value: false, readonly: true, order: 410 },
        pump_cmd: { title: 'Расчётная команда насоса 502', type: 'switch', value: false, readonly: true, order: 420 },
        supply_temp_c: { title: 'Подача ГП 415, °C', type: 'temperature', value: 0, readonly: true, order: 430 },
        return_temp_c: { title: 'Обратка ГП 416, °C', type: 'temperature', value: 0, readonly: true, order: 440 },
        source_temp_c: { title: 'Общая подача 411, °C', type: 'temperature', value: 0, readonly: true, order: 450 },
        delta_t_c: { title: 'ΔT ГП дом', type: 'value', value: 0, readonly: true, order: 460 },

        valve_position_cmd: { title: 'Команда клапана 561, %', type: 'value', value: 0, readonly: true, order: 500 },
...
defineRule('hm2_502_gp_dom_inputs', {
    whenChanged: [
        CH.supply,
        CH.ret,
        CH.source,
...
defineRule('hm2_502_gp_dom_period_changed', {
...
defineRule('hm2_502_gp_dom_reset_fault', {
