...
- для 505 live 23.09.2026 подтверждён пользовательский термостат `NL_simple_thermostat_505`: источник `901.01_MSW_TH/Temperature`, WB-MSW v.4 slave 202 физически в **хозяйственном помещении**; насос `A03/K5` остаётся под ownership HHM3;
- текущий desired-state Котельной ещё не добавляет `NL_simple_thermostat_505` в Sprut; это отдельная UI-задача issue #66;
