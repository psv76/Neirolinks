...
# 05 31 Иволга 13 · HHM3 UI/diagnostics cleanup
# Installs only:
...
TARGET_COMMIT="85d54bf4135e1753507b22aa18cf5f329e75fba0"
...
# Expected currently installed 500 from PR #65 production payload ca7b84a.
BASE_SHA_500="fc71ce0255d46929854aea12f3c69fbeecdbe3b6d5b3f3cb0957b8a1c4421846"
# Accepted current 620 variants:
# - previous OFF-readback overlay 42e04d5
# - already synchronized UI version (target)
BASE_SHA_620_OFF_FIX="a33e62406d6ed146c497f270dbb303c074b61cb59a4fcb956a708c18c43abbc2"

TARGET_SHA_500="dbad805288bdfbc1ea066e0f49912cc2ed82d3db44b5ab379bacde84fcf3d420"
TARGET_SHA_620="cfee47376aa8443a7bbf9ca843aa8ad9a78858e41b268fe93873a6bb189f49b5"
...
verify_live() {
    ...
    if [ "$s500" != "$BASE_SHA_500" ] && [ "$s500" != "$TARGET_SHA_500" ]; then
        echo "STOP: unexpected 500_HHM3_FSE.js SHA: $s500" >&2
        exit 1
    fi

    if [ "$s620" != "$BASE_SHA_620_OFF_FIX" ] && [ "$s620" != "$TARGET_SHA_620" ]; then
        echo "STOP: unexpected 620_thermostats.js SHA: $s620" >&2
        exit 1
    fi
...
install_update() {
    ...
    work="$(mktemp -d /root/hhm3_ui_cleanup_XXXXXXXX)"
    backup="/root/hhm3_ui_cleanup_rollback_$(date +%Y%m%d_%H%M%S)"
    ...
    echo "circuit_501:   $(mqtt_once HHM3_FSE circuit_501)"
    echo "circuit_502:   $(mqtt_once HHM3_FSE circuit_502)"
    echo "circuit_503:   $(mqtt_once HHM3_FSE circuit_503)"
    echo "circuit_504:   $(mqtt_once HHM3_FSE circuit_504)"
    echo "circuit_505:   $(mqtt_once HHM3_FSE circuit_505)"
    ...
    echo
    echo "=== RECENT HHM3 LOG ==="
    journalctl -u wb-rules --since "2 minutes ago" --no-pager -l | \
      grep -E '\[отопление\]|ERROR:|exception|Exception' | tail -100 || true
}

rollback_update() {
    [ "$#" -eq 1 ] || { echo "Usage: $0 rollback /root/hhm3_ui_cleanup_rollback_YYYYMMDD_HHMMSS" >&2; exit 2; }
    local backup="$1"
    need_cmd systemctl
    need_cmd mosquitto_sub
    preflight
